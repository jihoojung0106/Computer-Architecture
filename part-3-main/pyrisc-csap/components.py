...
class ALU(object):
    ...
    def op(self, alufun, alu1, alu2):
        ...
        if alufun == ALU_ADD:
            ...
            r=WORD((alu1>>20)&0x3ff)
            if r >= 0x100:
                r=0xff
            g=WORD((alu1>>10)&0x3ff)
            if g >= 0x100:
                g=0xff
            b=WORD(alu1&0x3ff)
            if b >= 0x100:
                b=0xff
            output = WORD(alu2<<24 | r<<16 | g<<8 | b)
        elif alufun == ALU_SVBRDCST:
            output = WORD((alu1<<20)&0x3ff00000 | (alu1<<10)&0xffc00 | alu1&0x3ff)
        elif alufun == ALU_SVADDI:
            output = WORD((alu1+(alu2<<20))&0x3ff00000 | (alu1+(alu2<<10))&0xffc00 | (alu1+alu2)&0x3ff)
            ...
            r1=WORD((alu1>>20)&0x3ff)
            r2=WORD((alu2>>20)&0x3ff)
            r=WORD(r1*r2)
            if (r&0xff)<0x80:
                r=0+(r>>8)
            else:
                r=1+(r>>8)
            g1=WORD((alu1>>10)&0x3ff)
            g2=WORD((alu2>>10)&0x3ff)
            g=WORD(g1*g2)
            if (g&0xff)<0x80:
                g=0+(g>>8)
            else:
